...
def test_journal_filters_only_bite_in_the_third_band():
    geplant = _row("g", segment=Segment.SCHEDULE)
    assert trifft_filter(geplant, typ=[], status=[], journal=["dropped"])


def test_dropped_selects_rows_without_a_file():
    weg = _row("weg", segment=Segment.JOURNAL, relation="dropped")
    da = _row("da", segment=Segment.JOURNAL, relation=None)
    assert trifft_filter(weg, typ=[], status=[], journal=["dropped"])
    assert not trifft_filter(da, typ=[], status=[], journal=["dropped"])


def test_oneshot_selects_finished_at_jobs():
    ...
    assert trifft_filter(einmal, typ=[], status=[], journal=["oneshot"])
    assert not trifft_filter(zyklisch, typ=[], status=[], journal=["oneshot"])
